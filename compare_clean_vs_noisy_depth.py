...
def get_prediction_summary(method, model_path, model, x_batch, y_batch, ood, mask_batch=None, eps=0.0):
    ...
    summary = [{"Method": method.value, "Model Path": model_path,
        ...
        "Target": y, "Mu": mu, "Sigma": sigma,
        "Adv. Mask": mask, "Epsilon": eps, "OOD": ood}
        for y,mu,sigma,mask in zip(y_batch, mu_batch, sigma_batch, mask_batch)]
    return summary
...
if args.load_pkl:
    ...
    #df_image.to_pickle("noisy_cached_depth_results.pkl")
    df_image.to_pickle("clean_cached_depth_results.pkl")
