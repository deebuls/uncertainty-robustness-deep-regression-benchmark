...
def compute_predictions(batch_size=10, n_adv=9):
    ...
    df_pred_image = pd.DataFrame(
        columns=["Method", "Model Path", "Input",
            "Target", "Mu", "Sigma", "Adv. Mask", "Epsilon", "OOD", "aleatoric", "epistemic"])
...
def get_prediction_summary(method, model_path, model, x_batch, y_batch, ood, mask_batch=None, eps=0.0):
    ...
    mu_batch, sigma_batch, aleatoric_batch, epistemic_batch = predict(method, model, x_batch)
    mu_batch = np.clip(mu_batch, 0, 1)
    sigma_batch = sigma_batch.numpy()
    aleatoric_batch = aleatoric_batch.numpy()
    epistemic_batch = epistemic_batch.numpy()
    ...
    summary = [{"Method": method.value, "Model Path": model_path,
        ...
        "Adv. Mask": mask, "Epsilon": eps, "OOD": ood, "aleatoric":aleatoric, "epistemic":epistemic}
        for x,y,mu,sigma,mask,aleatoric,epistemic in zip(x_batch, y_batch, mu_batch, sigma_batch, mask_batch, aleatoric_batch, epistemic_batch)]
    return summary
...
def gen_disentaglement_adversarial_comparison(df_image):
    print(f"Generating disentaglement adversarial plot with ood=False")
    df = df_image[df_image["OOD"]==False]
    
    uncertainty_df = df[['Epsilon','aleatoric','epistemic']]
    melt_uncertainty_df = pd.melt(uncertainty_df, id_vars=['Epsilon'], var_name=['uncertain'])
    print (melt_uncertainty_df.head())
    melt_uncertainty_df['Mean'] = [np.array(x).mean() for x in melt_uncertainty_df.value.values]
    print (melt_uncertainty_df.head())
    
    sns.catplot(x='Epsilon', y='Mean', hue='uncertain', data=melt_uncertainty_df, kind="box")
    plt.savefig(os.path.join(output_dir, f"combined_adversairal_disentaglement_box.pdf"))
    plt.show()
...
def gen_disentaglement_comparison(df_image):
    
    df = df_image[df_image["Epsilon"]==0.0] # Remove adversarial noise experiments
    df_pixel = df_image_to_pixels(df, keys=["Target", "Mu", "Sigma", "OOD", "aleatoric", "epistemic"])
    df_by_image = df_pixel.groupby([df_pixel.index, "Method","Model Path", "OOD"])

    df_mean_unc_img = df_by_image["aleatoric"].mean().reset_index() #mean of all pixels in every method and image
    #sns.catplot(x="Method", y="aleatoric", hue="OOD", data=df_mean_unc_img, kind="box", whis=0.5, showfliers=False)
    #plt.savefig(os.path.join(output_dir, f"ood_aleatoric_box.pdf"))

    df_mean_unc_img = df_by_image["epistemic"].mean().reset_index() #mean of all pixels in every method and image
    #sns.catplot(x="Method", y="epistemic", hue="OOD", data=df_mean_unc_img, kind="box", whis=0.5, showfliers=False)
    #plt.savefig(os.path.join(output_dir, f"ood_epistemic_box.pdf"))

    
    uncertainty_df = df[['OOD','aleatoric','epistemic']]
    melt_uncertainty_df = pd.melt(uncertainty_df, id_vars=['OOD'], var_name=['uncertain'])
    print (melt_uncertainty_df.head())
    melt_uncertainty_df['Mean'] = [np.array(x).mean() for x in melt_uncertainty_df.value.values]
    print (melt_uncertainty_df.head())
    
    sns.catplot(x='OOD', y='Mean', hue='uncertain', data=melt_uncertainty_df, kind="box")
    plt.savefig(os.path.join(output_dir, f"combined_ood_box.pdf"))
    plt.show()
...
def predict(method, model, x, n_samples=10):
    ...
    if method == Model.Dropout:
        ...
        return mu, sigma, aleatoric, epistemic
...
#gen_disentaglement_comparison(df_image)
gen_disentaglement_adversarial_comparison(df_image)
""" ================================================== """
