...
#SBATCH --output depth_dropout%j.out # filename for STDOUT (%N: nodename, %j: job-ID)
#SBATCH --error depth_dropout%j.err  # filename for STDERR
...
python train_depth.py --model dropout --batch-size 32
date
end=`date +%s`
runtime=$((end-start))
echo $runtime
